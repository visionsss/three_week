...
def read_csv():
    data = pd.read_csv('./data/data_practice.csv', index_col=0)
...
def predict_x(data_new):
    ...
    for i in range(len(cols) - 1):
        f, a, b, x0, C, P = GM11(data_new.loc[range(2007, 2014), cols[i]].values)  # C<0.35,0.5,0.65, P>0.95,0.8
...
def finall_model(data):
    ...
    ss = StandardScaler().fit(data.loc[range(2007, 2014), :])
    data.loc[:, :] = ss.transform(data.loc[:, :])
    x_train = data.iloc[:-2, :]
    x_test = data.iloc[-2:, :]
    y_train = y.iloc[:-2, :]
    ss = StandardScaler()
    y_train = ss.fit_transform(y_train).reshape(-1, )
    ...
    plt.plot(y.loc[range(2007, 2014), "y"])
